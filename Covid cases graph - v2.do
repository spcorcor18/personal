...
if `graphst'==1 {
    ...
    keep if state=="`state'"
    // strip out spaces from state name so can be used as filename
    local state2: subinstr local state " " "", all
	...
	twoway (bar newcases2 date2, sort fcolor(538b) fintensity(50) lwidth(thin) barwidth(0.8)) (line casesma date2, lwidth(medthick)) ///
		   ...
		   xlabel(`firstdate'(5)`lastdate', labsize(vsmall) angle(ninety) valuelabel) xmtick(`firstdate'(1)`lastdate') ///
		   legend(order(1 "New cases" 2 "`days'-day rolling average") position(11) ring(0)) ///
		   xsize(8) ysize(4) title("`state' new cases") xtitle("") ytitle("") name(`state2'cases,replace)
    graph save `state2'cases, replace
	graph export `state2'cases.png, as(png) replace
	...
	twoway (bar newdeaths2 date2, sort fcolor(538p) lcolor(538p) fintensity(70) lwidth(thin) barwidth(0.8)) (line deathsma date2, lwidth(medthick) lcolor(dknavy)) ///
		   ...
		   xlabel(`firstdate'(5)`lastdate', labsize(vsmall) angle(ninety) valuelabel) xmtick(`firstdate'(1)`lastdate') ///
		   legend(order(1 "New cases" 2 "`days'-day rolling average") position(11) ring(0)) ///
		   xsize(8) ysize(4) title("`state' new deaths") xtitle("") ytitle("") name(`state2'deaths,replace)
    graph save `state2'deaths, replace
	graph export `state2'deaths.png, as(png) replace
	}
...
if `graphco'==1 {
    ...
    // strip out spaces from state and county name so can be used as filename
    local state2: subinstr local state " " "", all
    local county2: subinstr local county " " "", all
	...
	*replace newcases2=343 if date2==r(max)
	...
	twoway (bar newcases2 date2, sort fcolor(538b) fintensity(50) lwidth(thin) barwidth(0.8)) (line casesma date2,lwidth(medthick)) ///
		   ...
		   xlabel(`firstdate'(5)`lastdate', labsize(vsmall) angle(ninety) valuelabel) xmtick(`firstdate'(1)`lastdate') ///
		   legend(order(1 "New cases" 2 "`days'-day rolling average") position(11) ring(0)) ///
		   xsize(8) ysize(4) title("`county' County `state' new cases") xtitle("") ytitle("") name(`state2'`county2'cases,replace)
    graph save `state2'`county2'cases, replace
	graph export `state2'`county2'cases.png, as(png) replace
	...
	twoway (bar newdeaths2 date2, sort fcolor(538p) lcolor(538p) fintensity(70) lwidth(thin) barwidth(0.8)) (line deathsma date2, lwidth(medthick) lcolor(dknavy)) ///
		   ...
		   xlabel(`firstdate'(5)`lastdate', labsize(vsmall) angle(ninety) valuelabel) xmtick(`firstdate'(1)`lastdate') ///
		   legend(order(1 "New cases" 2 "`days'-day rolling average") position(11) ring(0)) ///
		   xsize(8) ysize(4) title("`county' County `state' new deaths") xtitle("") ytitle("") name(`state2'`county2'deaths,replace)	
    graph save `state2'`county2'deaths, replace
	graph export `state2'`county2'deaths.png, as(png) replace
	}
